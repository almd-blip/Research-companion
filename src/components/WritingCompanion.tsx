...
export default function WritingCompanion({
  ...
  return (
    <div className="space-y-4 font-sans text-stone-850 dark:text-stone-100" id="writing-assistant-module">
      {/* Calm Unified Header Card */}
      <div className="p-4 sm:p-5 bg-white dark:bg-stone-900 border border-stone-200/80 dark:border-stone-800 rounded-2xl space-y-4 shadow-2xs">
        {/* Main Writing Assistant Action Bar */}
        ...
        <div className="pt-3.5 border-t border-stone-150 dark:border-stone-800/80">
          <ResearchIntegrityBanner variant="inline" />
        </div>
      </div>

      {/* Analysis Tabs */}
      <div className="bg-white dark:bg-stone-900 border border-stone-200/80 dark:border-stone-800 rounded-2xl p-4 sm:p-5 space-y-4 shadow-2xs">
        <div className="flex items-center gap-1.5 border-b border-stone-200/60 dark:border-stone-800 pb-2.5 overflow-x-auto text-xs">
          <button
            type="button"
              ...
              {activeAnalysisTab === 'unsupported' && (
                <div className="space-y-4">
                  {analysisResult?.unsupportedClaims?.map((item: any, idx: number) => (
                    <div
                      key={idx}
                      className="p-5 bg-stone-50/80 dark:bg-stone-900/60 border border-stone-200/90 dark:border-stone-800 rounded-2xl space-y-6 text-sm shadow-2xs"
                    >
                      {/* Calmer Alert Header */}
                      <div className="flex flex-wrap justify-between items-center gap-2">
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-md text-xs font-mono font-bold bg-[#912A4A]/10 text-[#912A4A] dark:bg-rose-950/40 dark:text-rose-300 border border-[#912A4A]/25 dark:border-rose-900/40 tracking-wider uppercase">
                      ...
                      {/* Claim Statement */}
                      <div className="p-3.5 bg-white dark:bg-stone-950/80 rounded-xl border border-stone-200/70 dark:border-stone-800">
                        <p className="font-serif italic text-base sm:text-[17px] text-stone-900 dark:text-stone-100 leading-relaxed">
                          "{item.claimText}"
                        </p>
                      </div>

                      {/* Issue & Suggestion with space-y-6 vertical spacing */}
                      <div className="space-y-6 pt-1">
                        ...
                        <div className="p-4 bg-stone-100/70 dark:bg-stone-950/50 border border-stone-200/80 dark:border-stone-800 rounded-xl space-y-1.5">
                          <p className="text-xs font-mono font-bold uppercase tracking-wider text-[#912A4A] dark:text-rose-300">
                            Suggestion:
                          </p>
                          <p className="text-sm sm:text-base text-stone-800 dark:text-stone-200 leading-relaxed">
                            {item.recommendation}
              ...
              {activeAnalysisTab === 'supported' && (
                <div className="space-y-4">
                  {analysisResult?.supportedByLibrary?.map((item: any, idx: number) => (
                    <div key={idx} className="p-4 bg-stone-50/80 dark:bg-stone-900/60 border border-stone-200/90 dark:border-stone-800 rounded-2xl space-y-4 text-sm shadow-2xs">
                      <div className="flex justify-between items-center gap-2">
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-md text-xs font-mono font-bold bg-emerald-100/80 text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-300 border border-emerald-300/60 dark:border-emerald-800/40 uppercase tracking-wider">
                          Evidence Found in Library
                        </span>
                        <button
                          type="button"
                          onClick={() => handleInsertPaperCitation(item.paperTitle, idx)}
                          className="px-3 py-1.5 bg-[#912A4A] hover:bg-[#78223d] text-white rounded-lg text-xs font-sans font-medium flex items-center gap-1.5 transition-colors cursor-pointer"
                        >
                          {appliedItemIdx === idx ? <Check className="w-3.5 h-3.5" /> : <Quote className="w-3.5 h-3.5" />}
                          <span>{appliedItemIdx === idx ? 'Inserted' : 'Insert Citation'}</span>
                        </button>
                      </div>
                      <div className="p-3 bg-white dark:bg-stone-950/80 rounded-xl border border-stone-200/70 dark:border-stone-800">
                        <p className="font-serif italic text-base text-stone-900 dark:text-stone-100 leading-relaxed">
                          "{item.claimText}"
                        </p>
                      </div>
                  ...
                  {analysisResult && (!analysisResult.supportedByLibrary || analysisResult.supportedByLibrary.length === 0) && (
                    <div className="p-4 bg-stone-50 dark:bg-stone-950 rounded-xl border border-stone-200 dark:border-stone-800 text-sm text-stone-500">
                      No direct library citations mapped yet for current sentences.
                    </div>
                  )}

                  {!analysisResult && (
                    <div className="py-8 text-center text-sm text-stone-400 space-y-1">
                      <p>Click "Check Draft & Sources" above to map your draft's arguments directly against your stored library.</p>
                    </div>
                  )}
                </div>
              )}
              ...
              {activeAnalysisTab === 'contradictions' && (
                <div className="space-y-4">
                  {analysisResult?.contradictoryEvidence?.map((item: any, idx: number) => (
                    <div key={idx} className="p-4 bg-stone-50/80 dark:bg-stone-900/60 border border-stone-200/90 dark:border-stone-800 rounded-2xl space-y-4 text-sm shadow-2xs">
                      <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-md text-xs font-mono font-bold bg-amber-100/80 text-amber-900 dark:bg-amber-950/40 dark:text-amber-300 border border-amber-300/60 dark:border-amber-800/40 uppercase tracking-wider">
